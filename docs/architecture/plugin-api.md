...
- `lm` — `selectChatModels(sessionId?)` (`api-types.ts:117`). Gated by `lm`, **built-in only**. An explicit `sessionId` pins the model to that session's runtime; omit it for the active session.
