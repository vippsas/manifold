...
- `src/shared/plugins/icons.ts` — `PLUGIN_ICON_IDS` / `PluginIconId` / `isPluginIconId()`: the closed set of activity-rail glyph names a view may claim. Shared so the main-process parser and the renderer's glyph table cannot drift.
...
- `verdicts` — `listByProject(projectId, limit?)` returning recorded session `VerdictRecord[]` for one project oldest→newest, `listAll()` returning every **registered** repo's verdicts grouped by project as `ProjectVerdicts[]` (display names joined main-side via `groupVerdictsByProject`; verdicts for repos no longer registered are ignored, not deleted), `clearProject(projectId)` deleting them, and `verifyPullRequests()` re-checking captured open PR URLs and updating merged verdicts (`api-types.ts:219`). `listByProject`/`listAll` are admitted by `verdicts:read`; `clearProject` and `verifyPullRequests` re-check `verdicts:write` (like the agents methods, `gated-api.ts`). All **built-in only** — `listAll` is the all-projects read path the `manifold.statistics` Dashboard card uses instead of renderer IPC. A record's `metrics` (`VerdictMetrics`) carries optional PR cache fields (`prState`, `prCheckedAt`, `prMergedAt`, `prCheckError`) plus optional `tokenUsage` (`TokenUsage`, re-exported from `manifold` at `api-types.ts:7`) + `turns`; absent token usage ⇒ the runtime exposed none (n/a).

**Capability gating.** `CAPABILITIES` (`manifest.ts:8`) is the single source of truth:
...
`BUILTIN_ONLY_CAPABILITIES` (`manifest.ts:15`) marks `workspace:manage`, `agent:control`, `agent:spawn`, `lm`,
...
**Contributions.** `contributes` (`manifest.ts:55`) is declarative metadata read by the
host; runtime behavior still requires the matching API call:
- `commands: [{ command, title }]` (`manifest.ts:39`) — palette entries.
- `views: [{ id, title, description?, launcher?, icon?, type?, frameSources? }]` (`manifest.ts:20`), where `type` is `'webview'` (default) or `'tree'`. `icon` (`manifest.ts:31`) names one of a **closed host-owned glyph set** (`PLUGIN_ICON_IDS`, `src/shared/plugins/icons.ts:10`) for the activity rail; a plugin picks a name, not a drawing, so every rail icon keeps the host's stroke weight and theming. An unrecognised name is **dropped rather than rejected** (`src/main/plugins/manifest.ts:93`) — a plugin built against a newer host still loads, falling back to the generic `plugin` glyph. `frameSources` (`manifest.ts:36`) is a list of **exact https origins** the view's webview may embed in iframes — it widens that view's CSP `frame-src` only (validated strictly in `src/main/plugins/manifest.ts:22`; served per-view by `webview-protocol.ts:102`). The plugin must back the view with `registerWebviewViewProvider(id, …)` or `registerTreeDataProvider(id, …)` in `activate`.
- `configuration: { title?, properties }` (`manifest.ts:50`) — each property is `{ type: 'string'|'number'|'boolean', default?, description?, enum? }`, exposed to `configuration.get`.

Internal modules and plugin views are normalized to a single `PanelContribution`
(`contributions.ts:18`) for the "+ Apps" launcher, tagged with `source: 'internal' | 'plugin'`.
The renderer reads two different slices of that registry: the Apps launcher honours
`launcher`, while the activity rail's plugin group shows every plugin view regardless — so
`launcher: false` means "not in the Apps list", not "not in the UI".
...
- `ManifoldPluginManifest` — `manifest.ts:61`. The `package.json` contract: `name`, `publisher`, `version`, `engines.manifold` (or `vscode`), `main`, plus optional `displayName`, `description`, `activationEvents`, `contributes`, `capabilities`.
