...
- `AiServiceSettings` — `api-types.ts:60`. App-level AI-service settings (provider + OpenAI/Azure keys), shared with core consumers; returned by `manifold.transcription.get()`. Re-exported from `src/shared/watch-types.ts` for the legacy import path.
