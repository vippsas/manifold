...
- `verdicts` — `listByProject(projectId, limit?)` returning recorded session `VerdictRecord[]` for one project oldest→newest, `listAll()` returning every **registered** repo's verdicts grouped by project as `ProjectVerdicts[]` (display names joined main-side via `groupVerdictsByProject`; verdicts for repos no longer registered are ignored, not deleted), and `clearProject(projectId)` deleting them (`api-types.ts:227`). `listByProject`/`listAll` are admitted by `verdicts:read`; the destructive `clearProject` re-checks `verdicts:write` (like the agents methods, `gated-api.ts`). All **built-in only** — `listAll` is the all-projects read path the `manifold.statistics` Dashboard card uses instead of renderer IPC. A record's `metrics` (`VerdictMetrics`) carries optional `tokenUsage` (`TokenUsage`, re-exported from `manifold` at `api-types.ts:7`) + `turns`; absent ⇒ the runtime exposed no usage (n/a).
