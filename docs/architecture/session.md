...
- `src/main/session/session-usage-accumulator.ts` — `SessionUsageAccumulator`: per-session live token/turn tally for chat-mode (print-mode) turns, drained at termination.

Not detailed here: `shell-*` (Manifold's AI shell prompt/suggestions), `nl-command-translator.*`, and `verdict-recorder.*` (per-session run records). They are session helpers, not the lifecycle core. **Token-usage capture (#728):** interactive Claude is spawned with `--session-id <session.id>` (`session-creator.ts:136`) so its transcript is locatable; at termination `VerdictRecorder.onSessionTerminated` resolves usage (chat-mode accumulator first, else the transcript reader for `runtime === 'claude'`, wired in `app/index.ts`) into `VerdictMetrics.tokenUsage`/`turns`. Other runtimes record nothing (n/a). For this to survive a restart the session id is persisted to worktree meta (`session-creator.ts:190`, `session-meta-persister.ts`) and **restored** on rediscovery rather than reminted (`session-discovery.ts:103`) — which also keeps verdict records idempotent across restarts. Resume passes `--resume <session.id>` for interactive Claude when that transcript exists (`session-resume.ts:65`) so post-resume turns append to the same transcript. Because a normal quit kills PTYs without firing `agent:exit`, `before-quit` first calls `finalizeAllForQuitSync()` (`verdict-recorder.ts:190`, via `sessionManager.finalizeActiveVerdictsForQuit()` at `app-lifecycle.ts:90`) to write active sessions' usage synchronously (`VerdictStore.upsert` is a synchronous atomic write). One cross-cutting note: the zsh prompt generated by `shell-prompt.ts` seeds its segment toggles at spawn but re-sources the shared `~/.manifold/shell-prompt-segments.zsh` (`shell-prompt-config.ts:13`) whenever it changes, so prompt-segment settings reach already-running shells at their next prompt render (settings-handlers writes that file at boot and on updates).
