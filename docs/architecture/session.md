...
- `src/main/session/transcript-usage-reader.ts` — `readClaudeTranscriptUsage()`: sums token usage + counts human turns from Claude's on-disk JSONL transcript (deduped by `message.id`), located via the session-id we spawn with.
