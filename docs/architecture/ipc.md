...
- `src/main/ipc/usage-handlers.ts` — `agent:session-usage`, the agent tab's cost hover. Looks the session up, returns `null` for a missing session or a non-Claude runtime, and otherwise delegates to `readSessionCost()` (`session/session-cost.ts`). Deliberately reads only the on-disk transcript: the live accumulator that the verdict recorder drains is destructive (`SessionUsageAccumulator.take`), so serving a hover from it would steal usage the verdict still needs. See `session.md`.
...
- `src/main/ipc/git-handlers.ts` — `diff:*`, `pr:create`, and the mutating `git:*` channels (`git:commit`, `git:ai-generate`, `git:ahead-behind`, `git:resolve-conflict`, `git:pr-context`, `git:fetch`, `git:has-uncommitted-changes`), plus the workspace-scoped Source Control channels: read-only `git:workspace-status` (branch, optional upstream `ahead`/`behind`, and `staged`/`unstaged` groups for every repo checkout), `git:workspace-stage` / `git:workspace-unstage` / `git:workspace-discard` (a path array each, so a whole group is one call), `git:workspace-commit` (per-repo commit; its `stageAll` flag picks between the index-only `commitManagedWorktreeIndex` and the stage-all `gitOps.commit`), `git:workspace-checkout` (mode-aware switch, `checkout -b [startPoint]`, or detached checkout), `git:workspace-pull` (the serialized status-bar sync: `pull --ff-only`, then `push`, returning a `GitSyncResult` whose failure arm includes the failed command, reason, and output), and `git:workspace-file-diff` (one file's uncommitted diff + original, its `staged` flag choosing HEAD-vs-index or index-vs-worktree, for the editor's SCM-click diff). Checkout and sync operations emit `workspace:list-changed` so Source Control, the status bar, and workspace list refresh, including after a sync command failure that may have updated refs. The session-scoped comparisons (diff, PR target, ahead/behind, pr-context) use `baseBranchFor(session, project)` = `session.baseBranch || project.baseBranch`, so a no-worktree agent based off a selected branch compares against that branch; workspace status instead compares `@{upstream}...HEAD` for VS Code-style sync counts.
