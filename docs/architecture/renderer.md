...
- `src/renderer/components/home/` — the global **Dashboard** home-layer surface: `DashboardHomeView` (a full-screen overlay opened from `DashboardSidebarButton` via `onOpenDashboard` / the `view.dashboard` command) renders a host-owned card grid (`dashboard-cards.ts`) of summary tiles; selecting one drills into that module's plugin webview by view id (`PluginViewPanel`) with a back-to-grid control. The Worktrees panel is the first card (`DashboardHomeView.tsx:39`, `dashboard-cards.ts:44`).
