...
- `agent` → **Agent** — `AgentPanel` (`components/editor/editor-shell/dock-agent-panel.tsx:86`): renders a draft chat, an `OnboardingView` (no agent yet), an `AgentChatView` (non-interactive chat-mode), or an xterm `TerminalPane` (interactive runtime) depending on session state. The `OnboardingView` → `NewAgentForm` create flow reads the `useWorktrees` setting (threaded as `defaultUseWorktrees` via the panel state, like `defaultAgentMode`); its Advanced section shows a per-agent **"Use an isolated worktree"** toggle (hidden when "Continue on an existing branch or PR" is selected, which is inherently in-place). Unchecking it sets `SpawnAgentOptions.noWorktree`, so the agent runs directly in the repo on a new branch. When the pending agent will run in place and another in-place agent is already running in the repo, the form shows a non-blocking warning (`NewAgentForm.tsx`).
