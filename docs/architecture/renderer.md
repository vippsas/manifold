...
else hangs off it but a draft-chat row while one exists (`WorkspaceCard.tsx:242`). The card's
rows step 8px at a time — repo row at 16px, its files at 24px (`WorkspaceCard.tsx:191`,
...
one, `WorkspaceCard.tsx:183`), **any number open at once**, each remembered across launches as
