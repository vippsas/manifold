...
**A workspace row's glyph says which kind it is.** Rows are grouped by repo — a home
workspace heads its repo's group and the worktree workspaces cut off it nest beneath
(`sidebar/RepoGroup.tsx`, model in `sidebar/sidebar-groups.ts`) — so `WorkspaceGlyph` draws
