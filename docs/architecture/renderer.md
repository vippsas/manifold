...
- `src/renderer/components/AgentTabUsage.tsx` — `AgentTabUsage`: the agent tab's cost coin and the hover-loaded tooltip behind it (`agent:session-usage`). `AgentTabUsage.fixture.tsx` renders its priced / unpriced / no-usage states.
- `src/renderer/monaco-setup.ts` — wires `MonacoEnvironment.getWorker` to the per-language Vite `?worker` bundles, applies `monaco-ts-defaults`, and calls `loader.config({ monaco })`.
- `src/renderer/monaco-ts-defaults.ts` — `configureTypeScriptDefaults(monaco)`: turns off semantic TS/JS validation (the worker has no project graph) and enables `jsx`.
...
- `src/renderer/components/home/` — the global **Dashboard** home-layer surface: `DashboardHomeView` (a full-screen overlay opened by the `view.dashboard` command — command palette or the native View menu — and by the New Agent modal's "View all worktrees" link through `dockState.onOpenDashboard`; no window chrome button opens it) renders a host-owned card grid (`dashboard-cards.ts`) of summary tiles; selecting one drills into that module's plugin webview by view id (`PluginViewPanel`) with a back-to-grid control. The cards are Worktrees and Statistics (the latter an all-projects view via `verdicts.listAll()`); each card's headline numbers come from a thin `dashboard:*-summary` IPC (`DashboardHomeView.tsx:39`, `dashboard-cards.ts:65`). A card is only offered while an enabled plugin still contributes the view it drills into — the grid filters `CARDS` against `usePluginContributions()` (`DashboardHomeView.tsx:68`), so Statistics, which ships disabled (`defaults.ts:84`), shows no card that would open an empty panel. When the Statistics webview opens with cached open PR records, its bridge posts one automatic `verify-prs` request so the panel can refresh stale PR state without waiting for the manual button.
...
**A workspace row's glyph says which kind it is.** Rows are grouped by repo — a muted
`RepoGroupHeader` heads each repo and every workspace of it, the clone included, nests
beneath as a peer card (`sidebar/RepoGroup.tsx`, model in `sidebar/sidebar-groups.ts`) — so
the header wears the repo glyph while `WorkspaceGlyph` draws
a folder for a **home** workspace, a git branch for a **worktree** one, and two linked
frames for a **multi**-repo one (`workspaceGlyphKind`, `shared/workspace-types.ts`), keyed
off `projectIds.length` then `isWorktreeWorkspace`, whose marker is `worktreePaths` and not
`branchName`.
...
repo alone put it in *both*: a second workspace over the same folders (the footer's "+ New
Agent") adopted the clone's agent and became its `primarySession` (`App.tsx:107`), which
...
rows' activity dots lit for the one running agent (`WorkspaceRowLabel.tsx:51`).
...
prefix off the name, names the single extra repo of a two-repo workspace (`+1 kong`) or
falls back to a bare count beyond that (`+3`), and drops the segment entirely when the
name already *is* the repo, so a home workspace stays `vops` rather than `vops / vops`. **The repo segment is sized to its own text and the
...
(`WorkspaceRepoRow.tsx:133`), which is not a row, so the highlight costs the same whether
...
row of an open workspace card carries a `RepoFetchButton` (`WorkspaceRepoRow.tsx:94`) that
...
a `⋯` (`WorkspaceActionsGlyph`, `WorkspaceCard.tsx:221`) that opens
...
**The footer's "+ New Agent" (formerly the row menu's "New Workspace, Same Folders") is
deliberately not called "Copy to New Worktree."** The old
...
`GitSyncFailureDialog` with Git's reason;
its sole substantive action, **Show Command Output**, opens the captured commands and output in
an in-memory `Git Sync Output.txt` editor tab (`StatusBar.tsx:55`, `:88`; `AppShell.tsx:168`;
`GitSyncFailureDialog.tsx:34`; `git-handlers.ts:267`).
The remaining items keep the agent/file status and optional commit/PR/conflict actions
(`StatusBar.tsx:99`, `:114`).
...
panels) are **underline tabs**: the strip is `--bg-chrome` closed by a hairline along its
bottom edge (an inset shadow, not a border, so it cannot add to the strip's fixed
`--chrome-tab-height`), each tab fills that height flush against its neighbours, and an idle
tab draws no fill at all — nothing to seam against the strip, nothing to mistake for a control
(`.dock-tab:not(.dock-tab--icon)`, `styles/theme.css:518`). The tab you are in is its label at
full contrast over a solid 2px accent line inset along its bottom edge, where the strip meets
the panel; an unfocused group states its tab in `--text-muted` instead, and hover fills
`--list-hover-bg` (all scoped to `.dv-active-tab` in `styles/dockview-theme.css:280`). Two
earlier shapes are worth knowing, because both were tried and reverted: a tab filled edge to
edge in `--bg-chrome` over a `--bg-chrome-hi` gradient strip made each tab a block with a
visible seam, and an inset rounded pill (`--sidebar-active-bg` behind a 1px ring) read as a
*button sitting inside the tab* — form-control language, and the only outlined thing in a strip
whose real controls (`+`, `×`) are flat. dockview pads `.dv-tab` by `0.25rem 0.5rem` and wraps
the custom tab in a block-level `.dv-react-part`, so filling the strip takes dropping that
padding entirely (the gap between labels is `.dock-tab`'s own, which keeps the hover fill and
the underline spanning the whole tab) and stretching the wrapper with flex boxes — `height:
100%` there resolves to `auto` against a flex item. Icon and headless tabs (sidebar/editor)
keep their own accent-square treatment. Each resize
...
group's share of the theme gap (`dock-layout-sidebar-width.ts:88`), which the pinned pass shaves off;
...
the file tabs and the agent tabs one panel over read as one row of tabs: the same flat
`--bg-chrome` strip closed by a hairline (`CodeViewer.styles.ts:24`), tabs filling its 30px
height and drawing nothing at rest, `--list-hover-bg` under the pointer, and the file you are in
marked by its label at full contrast over the same 2px accent underline (`.code-tab`,
`styles/theme.css:665`). A tab whose context menu is open says so the same way — both states set
the one `box-shadow`, since they can co-occur and the later rule would otherwise win outright
(`CodeViewerTabs.tsx:233`). Tabs carry no divider between them: an idle tab draws nothing, so
there is no edge for a seam to separate. A tab's `×` is reserved space but shows only on hover and on the active
tab — one per tab, always on, was a row of noise beside the names (`styles/theme.css:693`).
...
(`styles/theme.css:639`). **Every control in a header strip takes that same pill** — a text
tab's `×`, the icon-tab group's `×` (`styles/theme.css:529`) and the shell's `+`, which is
...
(`hooks/dock-layout/dock-layout-helpers.ts:103`) — hiding every other pane and the sidebar
...
`hooks/dock-layout/dock-layout-model.ts:35`): the sidebar is the full-height left column,
...
`hooks/dock-layout/dock-layout-loader.ts:226`) — removing a pane can leave dockview's root
...
axis (`hooks/dock-layout/dock-layout-loader.ts:183`, `:212`). Replaying the snapshot's
...
`hooks/dock-layout/dock-layout-loader.ts:94`). One sixth is therefore the sidebar's width
...
(`useDockLayout.ts:258`). Inside it, `dock-layout-helpers.ts` is the entry point: it
re-exports the subsystem's public surface so callers have one import, and holds the grid
geometry that belongs to no single module. The implementation is split by concern —
`dock-layout-model.ts` (panel ids, titles, homes, serialized grid types; constants and pure
readers only, so every other module can depend on it without a cycle),
`dock-layout-sidebar-width.ts` (pinning, measuring and restoring the sidebar column),
`dock-layout-loader.ts` (build, restore, show and hide), `dock-layout-shell-span.ts`
(widening the shell across the workspace row) and `dock-layout-sanitize.ts` (healing a
saved layout before it is restored).

**The panel set.** Panel ids are fixed in `PANEL_IDS` with display titles in
`PANEL_TITLES` (`hooks/dock-layout/dock-layout-model.ts:7`, `:12`):
...
- The **Explorer** view — `ProjectSidebar` (`ProjectSidebar.tsx:45`): a **Workspaces** toolbar — the label, an accent `+` right after it that opens the New Workspace modal (`sidebarStyles.toolbarNewButton`; beside the word it reads as "add one of these", not one more view toggle), then the right-aligned view toggles (`sidebarStyles.toolbarActions`) — then the body. The body is three sections at most — `FavoritesList`, `WorkingNowList`, and the **Repositories** tree — under that toolbar carrying a filter toggle, the sort toggle and a collapse-all control. **Working now** (`sidebar/WorkingNowList.tsx`) lists every workspace with a `running` or `waiting` agent, waiting first, and unmounts when empty; its ordering puts waiting agents first (`rowStatus`, `agent-labels.ts`). **Membership of that section comes from status; the dot on a row never does.** A row's dot is the theme accent and pulses (`.status-dot--active`, `core-pulse`), and it appears only while an agent there is *producing output* — `isOutputting`, dropped two seconds after the last chunk (`session-stream-wirer.ts`, idle timer) — which is also what drives the `.sidebar-label-working` sweep. So a workspace can be listed under Working now with no dot on it, because its agent is live but quiet. **`AgentStatus` is deliberately not wired to the dot**: `detectStatus` reports 'waiting' from the mere presence of a prompt character and *falls through to 'running' whenever nothing matches at all* (`agent/status-detector.ts:99`), and status is only recomputed when fresh PTY output arrives — so a status-driven dot stayed lit indefinitely on agents doing nothing, in the sidebar *and* in the status bar, which renders `status-dot--<status>` straight from the active session (`git/StatusBar.tsx`) and therefore shows the plain accent dot again. **Repositories** is `WorkspaceList` over `groupWorkspaces` (`sidebar-groups.ts`): one `RepoGroup` per primary repo (`projectIds[0]`), headed by a muted, unselectable `RepoGroupHeader` — **the repo is a fold, not a workspace** — with every one of its workspaces nested beneath it (`nested`: 14px step, indent guide, repo prefix dropped), the clone first and then its branches, all at one depth and merged branches behind a `MergedFold` (`workspace:list-merged`, verdict outcome `merged` only, never a live workspace). The header is deliberately not the clone. While it was, one chevron had to fold both the clone's own contents and the repo's other branches, so a clone's folder row and a branch card landed at neighbouring indents and the tree read as a flat run of look-alike rows, two of them repeating the repo's name; splitting them gives every chevron exactly one job. A clone whose name merely repeats its repo's reads as that repo's `baseBranch` instead (`main`) — the question the row is really answering once the header says the repo, and the only branch we know without a git call per repo — while a clone the user renamed keeps that name, which the base branch cannot carry. Every accessible name follows the *displayed* label, so a row reading `main` never announces `manifold` (`WorkspaceCard.tsx`, `displayLabel`). Groups order by the active group first then group recency, or A–Z by repo. **Any number of cards may be open, persisted** in `manifold.sidebar.openWorkspaces.v2` (`sidebar-fold-state.ts`, #902). The group fold belongs to the header (`repo:<projectId>`, or `repo:lone:<workspaceId>` when the primary repo is unregistered so closing a lone group cannot also toggle its one member's card); a card's own fold stays `workspace:<id>`. **Repos ship collapsed**, so the sidebar opens as an index of repos rather than a wall of every workspace at once; presence in the store means *open* for group and card keys alike, with no inversion. Entering a workspace opens its card and its group, so the sidebar always shows a row for where you are. The toolbar's third control **collapses every repo at once** (`collapseAllGroups`, which drops the `repo:`-prefixed keys and leaves each card's own fold alone) — the "give me the index back" gesture after a session of opening things. The toolbar's filter (`SidebarFilterField`) narrows the tree to substring hits on repo, workspace and branch, forces hit groups open, and hides Favorites and Working now while active. Clicking a card's row opens it as well as selecting it; the disclosure alone closes it again, so selecting a workspace never hides what is under it (`selectAndExpand`, `WorkspaceCard.tsx:122`, `:138`). **A card leads with a folder glyph, and that glyph *is* its disclosure** — one icon column, not a chevron beside a folder. It swaps to the state's chevron (right when closed, down when open) while the row is hovered or keyboard-focused, so the affordance appears where the eye already is; the swap is pure CSS on `.sidebar-workspace-toggle__folder` / `__chevron` (`theme.css:929`), keyed to `:hover` and `:focus-visible` rather than `:focus-within` so a click does not leave the chevron showing after the pointer has gone. **A workspace spanning exactly one repo renders no folder row** (`WorkspaceCardChildren.tsx`): that row could only repeat the repo's name the header already says, so the card's own disclosure opens the repo's files directly, one click instead of two, and the row absorbs what the folder row carried — the `↻` fetch pill in its action cluster, the fetch outcome as a line under the row, and Copy Path / Copy Relative Path appended to its menu (`buildWorkspaceContextMenu`'s `extraItems`). A **multi-repo** workspace keeps one folder row per repo, where the names differ and a row per repo is the only way to reach each one; a folder row is therefore now a signal that a workspace spans several repos. A card's folders keep their own open/closed files state while their card is closed, since that state is keyed by folder and persisted separately. **A card shows where work happens, never who is working: it renders no agent rows and no New-agent button.** Agents are the tabs of the Agent panel (below); the card signals a streaming agent on its own row twice — a pulsing accent `status-dot` beside the name, and `.sidebar-label-working` on the `repo / name` label, an accent highlight sweeping the text via the `sidebar-sweep` keyframe (`WorkspaceRowLabel.tsx:33`, `:44`, `:48`; `theme.css:1083`, `:2109`). The sweep is text-only (`background-clip: text` over a `currentColor` base, so the label reads normally between passes) and the row never moves or resizes; under `prefers-reduced-motion` it falls back to plain text rather than freezing the highlight mid-label (`theme.css:2282`). **The class goes on each segment of the path, never on the span wrapping them**: everything beneath one `background-clip: text` element is painted from that element's single gradient, so a wrapper flattened the dimmed repo to the row's own contrast and swallowed the `/` (which `opacity` isolates from an ancestor's background). A gradient per segment keeps each one's colour as the sweep's base — the band crosses a dimmed repo dimly and the name at full contrast. `background-attachment: fixed` is what still makes the three read as *one* band: without it each segment sizes its gradient to itself, so the band crosses a short repo faster than a long name and the `/` blinks on its own. Fixed also means the geometry is shared by every working row, so they sweep in step down the column, and the tile is a length (`360px` over `1.8s`) rather than a percentage — `shimmer` stays percentage-based for the chat thinking indicator, which sizes its gradient to itself (`ChatThinkingIndicator.tsx:57`). A folder row says nothing about agents either, and neither the card nor its folders name a branch — branch lives in Source Control. A card header carries the **`⋯`** that opens the worded action menu — plus, on a solo-repo card, the `↻` fetch pill (`WorkspaceActionsGlyph`, `WorkspaceCard.tsx:221`); double-clicking the name renames the workspace (`WorkspaceCard.tsx:193`). The actions themselves live only in that menu (see above), among them **Add Folder…** (native picker, attaches the chosen local project to that workspace). **Cutting a second workspace over the same folders is not in the menu; it is the footer's `+ New Agent`** (`ProjectSidebar.tsx:189`, `App.tsx:347`): to the user that action *is* a new, isolated agent, and as a row-menu item it read as a second, competing way to start one. It acts on the active (selected) workspace — disabled while none is — names the result after an unused Norwegian city, calls `workspace:create`, activates it, clears the session so the empty agent view greets the fresh checkout, and opens the New Agent dialog for it (`setNewAgentTarget`), so the harness is chosen before anything runs, defaulting to the source's runtime. **That cleared selection is sticky** (`useAgentSession.ts`, `clearedRef`): the hook's session list is per *repo* and its resync — on repo entry or `agent:sessions-changed` — used to treat an empty selection as "nothing chosen yet" and promote the repo's first or remembered session, which lives in another workspace; the new workspace then opened on that agent and `useAutoResume` restarted it. An explicit `setActiveSession(null)` now keeps the resync at `null` until any route selects a session again (sidebar, dock, or spawn, whose own resync lands before any effect runs, so the flag is lifted synchronously with the state). A folder row offers removal only while the workspace spans more than one, since the last one leaving would take the workspace with it (`WorkspaceCard.tsx:250`). **Clicking the card enters the workspace** (`onSelectWorkspace: enterWorkspace`, `App.tsx:416`): when the active agent isn't one of this workspace's, the main view jumps to one that is — two workspaces can span the same folders, so the active project alone can't tell them apart — or to the empty agent view when it has none. **Every way in shares that one answer**: the card row, a `FavoritesList` row, and ⌘1–9 all call `enterWorkspace` (`App.tsx:173`), which applies `resolveWorkspaceEntry` (`hooks/app/workspace-entry.ts`) — `keep` when the agent on screen already lives there, `agent` when the workspace has one of its own, `empty` when it has none. It is one function because the two used to disagree: a favorite click moved only the active workspace and folder, leaving the *previous* workspace's agent in the main view, so clicking a favorite looked like nothing had happened. Clicking a folder only opens its files and leaves the agent alone (`App.tsx:404`) — it cannot move an agent, which always runs in the workspace's *first* folder whatever the sidebar has selected. **Both footer actions are words in a bar below that list, not rows in it** — `+ New Repo` then `+ New Agent` (`sidebarStyles.actions`, `ProjectSidebar.tsx:177`): the list scrolls, the bar does not, so the way to start work cannot scroll out of reach. They are drawn as controls rather than rows — `--control-height`, text behind a hairline that lights on hover (`.sidebar-new-agent-button` / `.sidebar-new-repo-button`, `theme.css:1326`) — because a glyph-and-label row was indistinguishable from the workspace rows above it. Only the agent hairline is accent-tinted; registering a repo is the prerequisite rather than the act, so `+ New Repo` wears the neutral `--control-border` and cedes the accent to the button beside it. `+ New Workspace` used to be the accent button down here and is now the `+` on the toolbar: the footer's accent belongs to the everyday act (a new agent), and a whole new set of folders is the rarer, header-level act. **Adding a repository used to be a folder-plus glyph in the top toolbar and is not** — icon-only above a column of folder-looking rows, it read as "new workspace" and doubled the button below, so it moved down here and said what it does in words. Both refuse to shrink below a `120px` basis and wrap to a stack instead (`footerButton`), so a narrowed sidebar never clips `+ New Agen…`. The accent hairline deliberately stops short of the metal plate (`.btn-metal`) the primary agent CTA owns. A workspace entry in `FavoritesList` still leads with the cards' folder glyph (`WorkspaceGlyph`), and **right-clicking it offers `Remove from Favorites`** (`FavoritesList.tsx:61`) — removal is the row's whole vocabulary, since every other workspace action belongs to the card that owns those folders. Without it the only way out was the workspace's own card further down a scrolling list, which left a favorite reading as stuck. Once at least one repository exists, adding a repository and creating a workspace use body-portaled dialog overlays (`AddRepositoryModal`, `NewWorkspaceModal`), leaving the dock and current agent view mounted underneath; only the true first visit with zero repositories uses the full-pane `OnboardingView`. A folder row carries a manual fetch control (`RepoFetchButton`, `WorkspaceRepoRow.tsx:94`) — and so does a solo-repo card's own row, whose fetch state is owned by the card because the pill sits in the hover cluster while its outcome reads as a line under the row — on top of `useBranchStaleness`, which refreshes the active repository's remote-tracking state on activation and window focus, throttled to once every three minutes. Right-clicking a folder row opens the row's own two-item menu — **Copy Path** then **Copy Relative Path** (`buildRepoRowContextMenu`, `repo-row-context-menu.ts`), copying the row's effective path (the workspace's worktree checkout when it has one, else the registered clone, `WorkspaceRepoRow.tsx:53`) absolute or tilde-shortened; the shortening compares against the *real* home directory the preload captures once and exposes statically (`window.electronAPI.homeDir`, `src/preload/index.ts:230` — see [preload](preload.md)), with an exact path-boundary match (`tildePath`, so `/Users/median` is never under `/Users/me`) and falling back to the absolute path when home is unknown; both items render disabled when the row has no known path. The card's menu cannot open alongside: the card header is a *sibling* of its folder rows (`WorkspaceCard.tsx:137`, `:227`), so its handler is never in the propagation path — `useContextMenu.open`'s `stopPropagation` (`useContextMenu.ts:30`) belts that anyway, and once a menu is open, its body-portaled full-screen overlay swallows the next right-click and closes it (`ContextMenu.tsx:63`). A **multi-repo** card's own menu carries no copy-path items — it spans several folders, so "the" path is not well-defined there; a **solo-repo** card inherits them, since it stands in for the folder row it no longer renders and is then the only place that answers where the checkout is. **A workspace row carries no favorite star; favoriting lives on its right-click menu** (`WorkspaceCard.tsx:261`, built by `buildWorkspaceContextMenu`, `workspace-context-menu.ts`). A star was not an option rather than merely unwanted: the row's action cluster is `opacity: 0` at rest (`.sidebar-item-actions`, `theme.css:1036`), and opacity on a parent makes a compositing group a child cannot escape, so a star there could never stay lit to *mark* a favorite. Because the menu is the row's only such affordance it carries the whole set — the favorites toggle, then `Rename…`, `Add Folder…`, then `Remove Workspace` — omitting any whose handler is absent and collapsing the separators that leaves behind (`tidy`). Each card owns its own menu state (`useContextMenu`), so `Rename…` reuses the inline name draft the row already holds instead of lifting it into `WorkspaceList`. `ContextMenu` itself is shared with the file tree from `components/common/ContextMenu.tsx` and portals to `document.body`, since a `position: fixed` menu inside a dockview panel is otherwise offset by `.dv-render-overlay`'s transform. **Favorites are workspace ids and nothing else** (`settings.favorites: string[]`): every sidebar root is a workspace, and a repo has no root row of its own to star. A favorited workspace still appears in the list below as well as in `FavoritesList` — the section is a stable address with a fixed ⌘1–9 key, while the list below keeps reshuffling by recency in the default sort mode (less of a difference once that list is switched to A–Z, where every row's position is already stable). Every folder row is also the disclosure for its **files** — see below.
  ...
  (`AppShell.tsx:314`). The footer's "+ New Agent" remains a parallel checkout because its
  direct `createWorkspace` call omits that marker (`App.tsx:347`).
- The **Source Control** view — `SourceControl` (`components/git/SourceControl.tsx`, mounted by `SourceControlView`, `dock-panels.tsx:98`): a VS Code-style SCM view of the **active workspace**, one collapsible section per member repo checkout — the repo's name, its checked-out branch, a **⟳ / ✓ action row**, and its uncommitted changes under VS Code's three group headers, **Staged Changes**, **Changes**, and **Untracked Changes** (`components/git/ScmChangeGroup.tsx`), each with a count and colored M/A/D rows. Untracked files are sectioned off because discarding one deletes it outright, which the confirm dialog says rather than promising a revert. A file staged and then edited again appears in *both* groups: that is git's own model, which `git:workspace-status` preserves by returning `staged` and `unstaged` separately rather than one merged list. Group headers carry stage-all / unstage-all / discard-all; a row reveals its own +/−/↺ on hover, so a long list stays quiet until you reach for it. **Every action is also on a right-click menu** — repo header, group header, directory row, and file row each build their own from the shared `ContextMenu` (`components/common/ContextMenu.tsx`), which is where the actions with no hover affordance live. The panel toolbar's one control **toggles the changes between a flat list and a tree** (`components/git/scm-file-tree.ts`), VS Code's SCM view modes: the tree gives each directory a row that stages or discards everything beneath it, and compresses a chain of single-child directories onto one row (`src/components/git`) so a deep path costs one line rather than three. The mode is panel-wide, so every repo section switches together. Clicking a row is VS Code's SCM click: it opens the file in the editor **diffed against the right base for the half that was clicked** — a staged row against HEAD, an unstaged row against the index. The open request (`file-open-request.ts`) carries source `'sourceControl'` plus the checkout coordinates *and a `staged` flag*, `EditorPanel` (`dock-panels.tsx`) swaps its diff feed from the session's base-branch diff to `useWorkspaceFileDiff` (→ `git:workspace-file-diff`), and `useCodeViewerModes` opens such requests in diff mode. The data comes from one workspace-scoped IPC call, `git:workspace-status` (`git-handlers.ts`), which reads the workspace's own checkout of each repo (worktree, or the clone on a home workspace). `useWorkspaceRepoStatuses` keeps the last model **per workspace**, so returning to one paints its known rows in the selection render while a fresh status runs in the background; a first visit renders no other workspace's rows (`workspace-git-status.ts:23`, `:47`, `:58`). The active workspace revalidates on `files:changed`, `workspace:list-changed`, every workspace switch, window focus, **and its own 3s poll** (`workspace-git-status.ts:8`, `:71`). The poll is what makes the panel trustworthy: `files:changed` is emitted only for the paths `FileWatcher` polls — session worktrees and `--add-dir` folders (`file-watcher.ts:150`, `:190`) — and a workspace checkout is none of those, so a save in our own editor (or an external edit while the window keeps focus) pushed no event at all and the rows sat stale until some action's own `refresh()` happened to correct them. A tick is skipped while `document.hidden`, and while a status for that workspace is still in flight, so the git processes never stack up. Every section carries VS Code's **message box**, clean checkout or not: a per-repo commit input (⌘⏎, the Commit button, or the header's ✓ → `git:workspace-commit`). It is unconditional so it never moves out from under a half-typed message when the last change is staged or discarded. With something staged it commits exactly the index; with nothing staged it raises a `ConfirmDialog` first, since committing everything is a different act from the one a staging UI implies — confirming sends `stageAll`, which runs the same stage-all managed commit the Commit overlay uses. Discard is likewise confirmed: throwing away uncommitted work is unrecoverable, and one dialog hoisted to the panel serves every row and group. The **branch label is a pill button** — a bordered control rather than a caption, because a label nobody reads as clickable is a feature nobody finds (`components/git/BranchSwitcher.tsx`): clicking it opens a VS Code-style quick-pick — filter input, the repo's branches from the existing `git:list-branches` (which already hides branches held by other worktrees), and a "Create new branch" entry when the typed name matches nothing — checking the workspace checkout out via `git:workspace-checkout`; failures (e.g. dirty-tree conflicts) render inline and leave it open. It is a **centered modal**, not a popover anchored to the label: the panel is a narrow sidebar column that cramped the list and clipped long branch names. It reuses `createDialogStyles` + `useAutoFocus` and the Command Palette's keyboard model (↑/↓ move the active row, Enter selects, Escape closes), names its repo in the header since a workspace has several, and is portaled to `document.body` so an ancestor `transform` can't become the containing block for its fixed overlay. It is reached by selecting Source Control in the activity rail, the `view.sidebar.sourceControl` command (⌘⇧G), or the View menu — all of which swap the one sidebar onto this view.
  The picker now keeps the three VS Code top-level actions visible: **Create new branch…**,
  **Create new branch from…**, and **Checkout detached…**, followed by separate **Branches**
  and **Remote branches** sections. Create-from collects a name and then a start ref; detached
  checkout selects a ref without creating a local branch. The status-bar trigger uses this same
  component with flat chrome. `git:workspace-status` also returns optional upstream
  `ahead`/`behind` counts for that trigger; absence means no configured upstream resolved
  (`BranchSwitcher.tsx:143`, `:162`; `git-handlers.ts:153`, `:163`).
...
- `agent` → **Agent** — `AgentPanel` (`components/editor/editor-shell/dock-agent-panel.tsx:86`): renders a draft chat, an `OnboardingView` (no agent yet), an `AgentChatView` (non-interactive chat-mode), or an xterm `TerminalPane` (interactive runtime) depending on session state. **The group's tab bar is where agents live and are managed.** One tab per agent **of the active workspace**, not of the selected folder (`App.tsx` groups them by `workspaceId` and hands `useAgentSiblingDockTabs` that list) — a workspace's agents share its checkout, so hiding one behind a folder click would hide agents that work in the very same place; a **+** in the tab bar (`AgentHeaderActions.tsx:54`, mounted through `LeftHeaderActions` beside the shell's own +, `AppShell.tsx:54`) opens `NewAgentModal` on the active workspace via `onNewAgentFromHeader(workspace.id)` — falling back to whichever workspace holds the active repo when none is focused. It is deliberately *not* a runtime dropdown: the dialog owns the provider and Terminal/Chat choice, so the tab bar offers one affordance and one dialog wherever a new agent is started. At the **far right** of the strip (the right header slot, so it lands top-right of the view like the shell's ×) `AgentCloseHeaderActions` renders a `×` that **hides the active *sibling* agent tab** (`onCloseSiblingPanel`) — the agent stays alive, so selecting it again from the sidebar reopens its tab; it is disabled while the structural primary `agent` tab is active, which cannot be hidden. Both the + and the × fill their `.dv-react-part` wrapper and centre their 24px pill in it (`AgentHeaderActions.tsx:14`), because dockview's wrapper is block-level and would otherwise leave the control 6px high in the 30px strip. **A Claude tab also carries a cost coin.** `AgentTabUsage` (`components/AgentTabUsage.tsx:76`) sits between the label and the `…`, visible at rest like the tab's other controls, and its tooltip answers what the session has cost. It reads `agent:session-usage` **on hover** rather than on a timer, so an idle tab costs nothing and the figure is current whenever it is on screen; re-hovering refetches but keeps the last number on screen so it does not strobe. The headline is an estimate derived from published API rates — Claude records tokens, not prices — which the second line marks (`est. at API rates`), and an unrecognised model reads `Cost unavailable` rather than a guess. **A per-model table carries the breakdown** (`Breakdown`, `AgentTabUsage.tsx:48`): one row per model with input, output, cache read, cache write and its own cost, dearest first, closed by a **Total** row (`Totals`, `AgentTabUsage.tsx:81`) when more than one model ran. The total is not decoration: cells are rounded to fit the width, so adding the visible numbers does not reconcile — 22,823 and 16,738 read as `22.8k` and `16.7k`, summing to 39.5k against a true 39.6k — and the row does that addition from the exact figures. A single-model session skips it, since the one row already is the total. This is what makes a large total legible — a one-word prompt still bills tens of thousands of tokens, because cache traffic is the system prompt, CLAUDE.md, tool definitions and prior turns, and it dwarfs anything typed. The summary line therefore reports **two** figures, because conflating them is what made the total look wrong: `billed` is cumulative across every request (each turn re-reads the cached prefix and pays again), while `context` is the live window — `contextTokens`, the newest request's own input + cache read + write — which matches the status line's `Ctx` exactly. A real session read `103.9k billed · 42.5k context`. A `/model` switch widens the gap further: caches are model-scoped, so the new model re-writes the whole prefix instead of reading it, which the table makes visible as a row with large `Cache w` and zero `Cache r`. The coin is Claude-only (`DockTab.tsx:107`): no other runtime's transcript carries the per-model token split cost needs. **Both tab controls sit at rest, not behind a hover.** `.dock-tab__action` used to be `opacity: 0` until `.dock-tab:hover`/`:focus-within` (`styles/theme.css:555`), which kept the strip quiet but hid the affordances from anyone not already reaching for them — the coin inherited that and was invisible until you hovered the very tab whose cost you were trying to find. **Per-agent settings, lock and delete live in one overflow menu on each tab, not as separate glyphs** (`DockTab.tsx:87`, `:108`). The tab carries a single `…`; its worded menu opens `AgentSettingsModal`, toggles the agent's **deletion lock** through `onToggleLocked` → `agent:set-locked`, or routes to the delete-confirm dialog through `onRequestDeleteAgent`. Delete remains visible but disabled while the agent is locked, and the lock action becomes **Unlock agent** (`DockTab.tsx:88`, `:97`; `ContextMenu.tsx:73`). `requestDeleteAgent` (`hooks/app/useAppOverlays.ts:79`) is the single chokepoint every delete entry point funnels through and returns early for a locked session, so the lock also holds for the Delete Agent command and the New Agent view's reusable-sessions card; the main process refuses too. Deleting the agent is distinct from the header ×, which only hides its tab. Hiding a sibling is tracked in a module-level dismissed set (`hooks/agent-session/dismissed-agent-tabs.ts`) so the auto-tab reconciler (`useAgentSiblingDockTabs`) — which re-runs on every dock change — doesn't immediately recreate the hidden tab; reopening (select, or `openSiblingPanel`) clears the mark. **The form asks only what the workspace doesn't already answer.** Starting an agent is
...
`SidebarCardActionGlyphs.tsx:107`). `WorkspaceList` passes the mode into `groupWorkspaces`
(`sidebar-groups.ts`), which does the actual ordering — at the group level, and, by calling
`sortWorkspaces` on each group's own worktrees and merged branches, at the member level too
(`sidebar-groups.ts:92`, `:93`). The choice persists at `manifold.sidebar.sort.v1`, and a storage
failure costs only the restore, not the toggle (`sidebar-sort.ts:31`).

**A–Z sorts groups by repo name**, then always heads each group with its home workspace
(`groupWorkspaces`'s alpha branch, `sidebar-groups.ts:96`–`:98`): the home is a fixed field on
the group, not a member competing for a row, so it never sorts alongside its own worktrees.
Within a group the worktree and merged branches sort by their own name via the same
`workspaceRowLabel`-keyed comparator (`sidebar-sort.ts:53`), so the order on screen can never
disagree with what a row reads. **Nothing is pinned in A–Z:** the mode exists to make a name's
position predictable, which a row floating to the top on entry would undo. The sticky
active-card header below is unaffected — it is scoped to the card, so it still holds the active
row in view while that card's own content scrolls, wherever the card sits.

In `recency`, **the group holding the active workspace sorts first**, not the workspace's own
row: `groupWorkspaces` ranks groups by whether they hold the active workspace, then by the most
recent touch among any of their members (`sidebar-groups.ts:99`–`:102`). Inside a group, the
worktree and merged branches then sort by their own recency with no member-level pin —
`sortWorkspaces` is called with `activeId: null` for group members (`sidebar-groups.ts:89`) —
while the home workspace, again, is a fixed field that always heads the group rather than
joining that sort. Moving to another workspace therefore moves its *whole family* to the top of
the group order; which group ends up second depends on which one you were already in, not a
guaranteed one-click-back row. The active workspace's own card **sticks** while its folders and
files scroll under it, so going deep into a tree never costs you the label saying where you are;
it needs an explicit background, since a row is transparent until hovered and the rows passing
beneath would read straight through it
...
Within a group, the visit that reorders its members is recorded from whichever workspace ended
up *active* rather than from the click that asked for it — opening a folder inside another
workspace, or a session restored at launch, moves you just as a click on the row does, and all
of them have to leave the same trail (`WorkspaceList.tsx:74`, `sidebar-recency.ts:48`). Only the
row you moved to and the one you left change places within their group: every other row keeps
the order it was already in, because their timestamps don't move and the sort is stable.
...
those read as clutter (`ProjectSidebar.styles.ts:68`, `styles/theme.css:693`). The whole
...
**Diagnostics are syntax-only.** The TS worker sees one file at a time — no tsconfig, no
`node_modules`, no sibling modules — so every *semantic* answer it gives comes out of an empty
project: `Cannot find module 'react'`, `Cannot find name 'Deno'`, implicit-`any` on annotated
parameters. `configureTypeScriptDefaults` therefore sets `noSemanticValidation: true` for both
the `typescript` and `javascript` defaults and leaves `noSyntaxValidation: false`, so a file
with a real unbalanced brace still squiggles while a perfectly good Deno or React file reads
clean (`monaco-ts-defaults.ts:13`). Re-enabling semantic checks means giving the worker a real
file graph (a language server), not flipping the flag back.

Syntax validation is only trustworthy because the model is *named after the file*:
`EditorContent` passes `path={editorModelPath(paneId, filePath)}`
(`components/editor/editor-shell/EditorContent.tsx:36`). Monaco reads a model's script kind off
its URI extension, not off the language id — and `.tsx`/`.jsx` map to the plain
`typescript`/`javascript` ids — so at the default `inmemory://model/1` URI every JSX tag parsed
as a failed type assertion and reported a syntax error. The pane id keeps the URI unique:
`@monaco-editor/react` looks models up by URI and disposes the model when its editor unmounts,
so two split panes on the same file must not share one
(`components/editor/editor-shell/editor-model-path.ts:13`).
...
`handleOpenTransientFile` adds an editor tab without a disk read; these tabs carry an unsaved
dot, keep edits in memory, are excluded from file refresh and session view-state persistence,
and never invoke `files:write` (`hooks/editor/useCodeViewFileOps.ts:170`,
`hooks/agent-session/useSessionStatePersistence.ts:31`,
`components/editor/code-viewer/CodeViewerTabs.tsx:250`). Git sync failure output uses this path.
...
- `PANEL_IDS` / `PANEL_TITLES` — `hooks/dock-layout/dock-layout-model.ts:7`, `:12`. Canonical panel id and title lists.
...
- **A Monaco model URI must end in the file's real extension.** The TS worker takes the script kind from the URI, so a `.tsx` file at an extension-less model URI reports JSX as a syntax error. `editorModelPath` builds `inmemory://<paneId>/<file>` for exactly this; dropping the `path` prop from `EditorContent` brings the false errors back.
