...
- The **Explorer** view — `ProjectSidebar` (`ProjectSidebar.tsx:38`): a **Workspaces** toolbar, right-aligned (`sidebarStyles.toolbarActions`), then the body. The body is three sections at most — `FavoritesList`, `WorkingNowList`, and the **Repositories** tree — under that toolbar carrying a filter toggle, the sort toggle and a collapse-all control. **Working now** (`sidebar/WorkingNowList.tsx`) lists every workspace with a `running` or `waiting` agent, waiting first, and unmounts when empty; its ordering puts waiting agents first (`rowStatus`, `agent-labels.ts`). **Membership of that section comes from status; the dot on a row never does.** A row's dot is the theme accent and pulses (`.status-dot--active`, `core-pulse`), and it appears only while an agent there is *producing output* — `isOutputting`, dropped two seconds after the last chunk (`session-stream-wirer.ts`, idle timer) — which is also what drives the `.sidebar-label-working` sweep. So a workspace can be listed under Working now with no dot on it, because its agent is live but quiet. **`AgentStatus` is deliberately not wired to the dot**: `detectStatus` reports 'waiting' from the mere presence of a prompt character and *falls through to 'running' whenever nothing matches at all* (`agent/status-detector.ts:99`), and status is only recomputed when fresh PTY output arrives — so a status-driven dot stayed lit indefinitely on agents doing nothing, in the sidebar *and* in the status bar, which renders `status-dot--<status>` straight from the active session (`git/StatusBar.tsx`) and therefore shows the plain accent dot again. **Repositories** is `WorkspaceList` over `groupWorkspaces` (`sidebar-groups.ts`): one `RepoGroup` per primary repo (`projectIds[0]`), headed by a muted, unselectable `RepoGroupHeader` — **the repo is a fold, not a workspace** — with every one of its workspaces nested beneath it (`nested`: 14px step, indent guide, repo prefix dropped), the clone first and then its branches, all at one depth and merged branches behind a `MergedFold` (`workspace:list-merged`, verdict outcome `merged` only, never a live workspace). The header is deliberately not the clone. While it was, one chevron had to fold both the clone's own contents and the repo's other branches, so a clone's folder row and a branch card landed at neighbouring indents and the tree read as a flat run of look-alike rows, two of them repeating the repo's name; splitting them gives every chevron exactly one job. A clone whose name merely repeats its repo's reads as that repo's `baseBranch` instead (`main`) — the question the row is really answering once the header says the repo, and the only branch we know without a git call per repo — while a clone the user renamed keeps that name, which the base branch cannot carry. Every accessible name follows the *displayed* label, so a row reading `main` never announces `manifold` (`WorkspaceCard.tsx`, `displayLabel`). Groups order by the active group first then group recency, or A–Z by repo. **Any number of cards may be open, persisted** in `manifold.sidebar.openWorkspaces.v2` (`sidebar-fold-state.ts`, #902). The group fold belongs to the header (`repo:<projectId>`, or `repo:lone:<workspaceId>` when the primary repo is unregistered so closing a lone group cannot also toggle its one member's card); a card's own fold stays `workspace:<id>`. **Repos ship collapsed**, so the sidebar opens as an index of repos rather than a wall of every workspace at once; presence in the store means *open* for group and card keys alike, with no inversion. Entering a workspace opens its card and its group, so the sidebar always shows a row for where you are. The toolbar's third control **collapses every repo at once** (`collapseAllGroups`, which drops the `repo:`-prefixed keys and leaves each card's own fold alone) — the "give me the index back" gesture after a session of opening things. The toolbar's filter (`SidebarFilterField`) narrows the tree to substring hits on repo, workspace and branch, forces hit groups open, and hides Favorites and Working now while active. Clicking a card's row opens it as well as selecting it; the disclosure alone closes it again, so selecting a workspace never hides what is under it (`selectAndExpand`, `WorkspaceCard.tsx:122`, `:138`). **A card leads with a folder glyph, and that glyph *is* its disclosure** — one icon column, not a chevron beside a folder. It swaps to the state's chevron (right when closed, down when open) while the row is hovered or keyboard-focused, so the affordance appears where the eye already is; the swap is pure CSS on `.sidebar-workspace-toggle__folder` / `__chevron` (`theme.css:929`), keyed to `:hover` and `:focus-visible` rather than `:focus-within` so a click does not leave the chevron showing after the pointer has gone. **A workspace spanning exactly one repo renders no folder row** (`WorkspaceCardChildren.tsx`): that row could only repeat the repo's name the header already says, so the card's own disclosure opens the repo's files directly, one click instead of two, and the row absorbs what the folder row carried — the `↻` fetch pill in its action cluster, the fetch outcome as a line under the row, and Copy Path / Copy Relative Path appended to its menu (`buildWorkspaceContextMenu`'s `extraItems`). A **multi-repo** workspace keeps one folder row per repo, where the names differ and a row per repo is the only way to reach each one; a folder row is therefore now a signal that a workspace spans several repos. A card's folders keep their own open/closed files state while their card is closed, since that state is keyed by folder and persisted separately. **A card shows where work happens, never who is working: it renders no agent rows and no New-agent button.** Agents are the tabs of the Agent panel (below); the card signals a streaming agent on its own row twice — a pulsing accent `status-dot` beside the name, and `.sidebar-label-working` on the `repo / name` label, an accent highlight sweeping the text via the `sidebar-sweep` keyframe (`WorkspaceRowLabel.tsx:33`, `:44`, `:48`; `theme.css:1083`, `:2109`). The sweep is text-only (`background-clip: text` over a `currentColor` base, so the label reads normally between passes) and the row never moves or resizes; under `prefers-reduced-motion` it falls back to plain text rather than freezing the highlight mid-label (`theme.css:2282`). **The class goes on each segment of the path, never on the span wrapping them**: everything beneath one `background-clip: text` element is painted from that element's single gradient, so a wrapper flattened the dimmed repo to the row's own contrast and swallowed the `/` (which `opacity` isolates from an ancestor's background). A gradient per segment keeps each one's colour as the sweep's base — the band crosses a dimmed repo dimly and the name at full contrast. `background-attachment: fixed` is what still makes the three read as *one* band: without it each segment sizes its gradient to itself, so the band crosses a short repo faster than a long name and the `/` blinks on its own. Fixed also means the geometry is shared by every working row, so they sweep in step down the column, and the tile is a length (`360px` over `1.8s`) rather than a percentage — `shimmer` stays percentage-based for the chat thinking indicator, which sizes its gradient to itself (`ChatThinkingIndicator.tsx:57`). A folder row says nothing about agents either, and neither the card nor its folders name a branch — branch lives in Source Control. A card header carries the **`⋯`** that opens the worded action menu — plus, on a solo-repo card, the `↻` fetch pill (`WorkspaceActionsGlyph`, `WorkspaceCard.tsx:221`); double-clicking the name renames the workspace (`WorkspaceCard.tsx:193`). The actions themselves live only in that menu (see above), among them **New Workspace, Same Folders** (`App.tsx:305` names it after an unused Norwegian city, calls `workspace:create`, activates the result and clears the session so the empty agent view greets the fresh checkout) and **Add Folder…** (native picker, attaches the chosen local project to that workspace). A folder row offers removal only while the workspace spans more than one, since the last one leaving would take the workspace with it (`WorkspaceCard.tsx:250`). **Clicking the card enters the workspace** (`onSelectWorkspace: enterWorkspace`, `App.tsx:416`): when the active agent isn't one of this workspace's, the main view jumps to one that is — two workspaces can span the same folders, so the active project alone can't tell them apart — or to the empty agent view when it has none. **Every way in shares that one answer**: the card row, a `FavoritesList` row, and ⌘1–9 all call `enterWorkspace` (`App.tsx:173`), which applies `resolveWorkspaceEntry` (`hooks/app/workspace-entry.ts`) — `keep` when the agent on screen already lives there, `agent` when the workspace has one of its own, `empty` when it has none. It is one function because the two used to disagree: a favorite click moved only the active workspace and folder, leaving the *previous* workspace's agent in the main view, so clicking a favorite looked like nothing had happened. Clicking a folder only opens its files and leaves the agent alone (`App.tsx:404`) — it cannot move an agent, which always runs in the workspace's *first* folder whatever the sidebar has selected. **Both create actions are words in a bar below that list, not rows in it** — `+ New Repo` then `+ New Workspace` (`sidebarStyles.actions`, `ProjectSidebar.tsx:112`): the list scrolls, the bar does not, so the only way to create a workspace cannot scroll out of reach — the pre-#880 arrangement, restored. They are drawn as controls rather than rows — `--control-height`, text behind a hairline that lights on hover (`.sidebar-new-workspace-button` / `.sidebar-new-repo-button`, `theme.css:1202`) — because a glyph-and-label row was indistinguishable from the workspace rows above it. Only the workspace hairline is accent-tinted; registering a repo is the prerequisite rather than the act, so `+ New Repo` wears the neutral `--control-border` and cedes the accent to the button beside it. **Adding a repository used to be a folder-plus glyph in the top toolbar and is not** — icon-only above a column of folder-looking rows, it read as "new workspace" and doubled the button below, so it moved down here and said what it does in words. Both refuse to shrink below a `120px` basis and wrap to a stack instead (`footerButton`), so a narrowed sidebar never clips `+ New Workspac…`. The accent hairline deliberately stops short of the metal plate (`.btn-metal`) the primary agent CTA owns. A workspace entry in `FavoritesList` still leads with the cards' folder glyph (`WorkspaceGlyph`), and **right-clicking it offers `Remove from Favorites`** (`FavoritesList.tsx:61`) — removal is the row's whole vocabulary, since every other workspace action belongs to the card that owns those folders. Without it the only way out was the workspace's own card further down a scrolling list, which left a favorite reading as stuck. Once at least one repository exists, adding a repository and creating a workspace use body-portaled dialog overlays (`AddRepositoryModal`, `NewWorkspaceModal`), leaving the dock and current agent view mounted underneath; only the true first visit with zero repositories uses the full-pane `OnboardingView`. A folder row carries a manual fetch control (`RepoFetchButton`, `WorkspaceRepoRow.tsx:94`) — and so does a solo-repo card's own row, whose fetch state is owned by the card because the pill sits in the hover cluster while its outcome reads as a line under the row — on top of `useBranchStaleness`, which refreshes the active repository's remote-tracking state on activation and window focus, throttled to once every three minutes. Right-clicking a folder row opens the row's own two-item menu — **Copy Path** then **Copy Relative Path** (`buildRepoRowContextMenu`, `repo-row-context-menu.ts`), copying the row's effective path (the workspace's worktree checkout when it has one, else the registered clone, `WorkspaceRepoRow.tsx:53`) absolute or tilde-shortened; the shortening compares against the *real* home directory the preload captures once and exposes statically (`window.electronAPI.homeDir`, `src/preload/index.ts:230` — see [preload](preload.md)), with an exact path-boundary match (`tildePath`, so `/Users/median` is never under `/Users/me`) and falling back to the absolute path when home is unknown; both items render disabled when the row has no known path. The card's menu cannot open alongside: the card header is a *sibling* of its folder rows (`WorkspaceCard.tsx:137`, `:227`), so its handler is never in the propagation path — `useContextMenu.open`'s `stopPropagation` (`useContextMenu.ts:30`) belts that anyway, and once a menu is open, its body-portaled full-screen overlay swallows the next right-click and closes it (`ContextMenu.tsx:63`). A **multi-repo** card's own menu carries no copy-path items — it spans several folders, so "the" path is not well-defined there; a **solo-repo** card inherits them, since it stands in for the folder row it no longer renders and is then the only place that answers where the checkout is. **A workspace row carries no favorite star; favoriting lives on its right-click menu** (`WorkspaceCard.tsx:261`, built by `buildWorkspaceContextMenu`, `workspace-context-menu.ts`). A star was not an option rather than merely unwanted: the row's action cluster is `opacity: 0` at rest (`.sidebar-item-actions`, `theme.css:1036`), and opacity on a parent makes a compositing group a child cannot escape, so a star there could never stay lit to *mark* a favorite. Because the menu is the row's only such affordance it carries the whole set — the favorites toggle, then `Rename…`, `New Workspace, Same Folders`, `Add Folder…`, then `Remove Workspace` — omitting any whose handler is absent and collapsing the separators that leaves behind (`tidy`). Each card owns its own menu state (`useContextMenu`), so `Rename…` reuses the inline name draft the row already holds instead of lifting it into `WorkspaceList`. `ContextMenu` itself is shared with the file tree from `components/common/ContextMenu.tsx` and portals to `document.body`, since a `position: fixed` menu inside a dockview panel is otherwise offset by `.dv-render-overlay`'s transform. **Favorites are workspace ids and nothing else** (`settings.favorites: string[]`): every sidebar root is a workspace, and a repo has no root row of its own to star. A favorited workspace still appears in the list below as well as in `FavoritesList` — the section is a stable address with a fixed ⌘1–9 key, while the list below keeps reshuffling by recency in the default sort mode (less of a difference once that list is switched to A–Z, where every row's position is already stable). Every folder row is also the disclosure for its **files** — see below.
  ...
  direct `createWorkspace` call omits that marker (`App.tsx:312`).
