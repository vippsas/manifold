...
- `src/main/git/porcelain-status.ts` — `parseWorkspaceStatus`: splits `git status --porcelain` into `staged` / `unstaged` / `untracked` by reading the index and worktree columns separately, for the Source Control panel's staging UI. `??` entries are their own group rather than part of `unstaged`, so a stage-all or discard-all over the tracked half leaves brand-new files alone.
