...
// Ported from src/renderer/components/watch/WatchPanel.tsx. Differences:
// - useDockState() → useWatchBridge() (sessionId arrives via the host `init`).
// - "Open agent" → bridge revealAgent (the host app owns panel placement; the
//   builtin's dock reference-panel swap logic is gone, openSiblingId remains
//   webview state only).
// - The builtin's `isRunning` gate (dock.activeSessionStatus) is dropped: the
//   webview can't see agent status; the host preflights it on run instead.
...
export function WatchPanel(): React.JSX.Element {
  ...
  }, [preview.entries, preview.selectedIndices, focusedEntryIndex, setFocusedEntryIndex])
  ...
  const canRun = !!sessionId && !busy && ready && pendingEntries.length > 0
  const canImprove = !!sessionId && improvingIndex === null && !busy
    ...
    <div ref={containerRef} style={s.container}>
      ...
      <WatchPlaylistPreview
        ...
        onOpenSibling={(index) => {
          ...
          if (!sid) return
          // The host app decides where the sibling panel opens; the webview
          // just asks for it to be revealed. openSiblingId is kept so state
          // restores match the builtin's shape.
          bridge.revealAgent(sid, preview.entries[index]?.title)
          if (openSiblingId !== sid) setOpenSiblingId(sid)
