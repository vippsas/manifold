...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    ...
    # Azure first — when a user has Azure configured we should prefer it over
    # any leftover OpenAI/Groq keys to avoid surprise charges on the wrong account.
    candidates = (
...
def _post_azure_whisper(api_key: str, audio_path: Path) -> dict:
    """Azure OpenAI Whisper uses a different URL shape and an `api-key` header.

    Endpoint: <AZURE_OPENAI_ENDPOINT>/openai/deployments/<deployment>/audio/transcriptions?api-version=...
    """
    endpoint_base = (os.environ.get("AZURE_OPENAI_ENDPOINT") or "").strip().rstrip("/")
    if not endpoint_base:
        # Fall back to dotenv lookup the same way load_api_key does
        for path in (Path.home() / ".config" / "watch" / ".env", Path.cwd() / ".env"):
            ...
            for line in path.read_text().splitlines():
                ...
                if line.startswith("AZURE_OPENAI_ENDPOINT="):
                    endpoint_base = line.split("=", 1)[1].strip().rstrip("/")
                    break
            if endpoint_base:
                break
    if not endpoint_base:
        raise SystemExit("Azure OpenAI: AZURE_OPENAI_ENDPOINT is required")
    ...
    try:
        ...
        detail = _read_error_body(exc)
        raise SystemExit(f"Azure Whisper request failed: {exc}{detail}")
    try:
        return json.loads(payload)
    except json.JSONDecodeError as exc:
        raise SystemExit(f"Azure Whisper returned non-JSON response: {exc}: {payload[:200]}")
...
def transcribe_video(
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            "No Whisper API key available. Set AZURE_OPENAI_API_KEY (with AZURE_OPENAI_ENDPOINT), "
    ...
    if backend == "groq":
        ...
        response = _post_azure_whisper(api_key, audio_path)
    else:
