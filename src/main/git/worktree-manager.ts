...
export class WorktreeManager {
  ...
  async createWorktreeFromBranch(
    ...
    if (fs.existsSync(worktreePath)) {
      await gitExec(['reset', '--mixed', 'HEAD'], worktreePath).catch((err) => {
        debugLog(`[worktree] reset --mixed HEAD failed for reused ${worktreePath}: ${err}`)
      })
      await prepareManagedWorktree(worktreePath)
      return { branch, path: worktreePath }
    }

    await gitExec(['worktree', 'prune'], projectPath).catch((err) => {
      debugLog(`[worktree] prune failed before add for ${projectPath}: ${err}`)
    })
