...
export const PANEL_IDS = ['projects', 'agent', 'editor', 'shell'] as const
...
export const PANEL_RESTORE_HINTS: Record<DockPanelId, Array<{ ref: DockPanelId; dir: Direction }>> = {
  projects: [{ ref: 'agent', dir: 'left' }, { ref: 'editor', dir: 'left' }],
  agent: [{ ref: 'editor', dir: 'left' }, { ref: 'projects', dir: 'right' }, { ref: 'shell', dir: 'above' }],
 ...
 *  Only the left (projects) sidebar exists today; the gesture engine stays
 *  side-agnostic so a future right sidebar can slot back in. */
export const SIDEBAR_PANEL_IDS = new Set<string>(['projects'])
...
const NON_WORKSPACE_PANEL_IDS = new Set<string>(['projects'])
...
/** Capture sidebar widths. Only the left (projects) sidebar exists today; the
 *  {left, right} shape is kept so the width-restore plumbing stays side-agnostic. */
...
export function restoreSidebarWidths(api: DockviewApi, widths: { left: number; right: number }, refs?: LayoutRefs): void {
  ...
  for (const [panelId, width] of [['projects', widths.left]] as const) {
    if (width <= 0) continue
...
export function restoreCollapsedSidebarWidths(api: DockviewApi, saved: SerializedDockview): void {
  for (const [side, panelId] of [['left', 'projects']] as const) {
