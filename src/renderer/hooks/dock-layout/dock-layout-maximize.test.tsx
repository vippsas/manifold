...
// must fill the dock area, hide every other group AND the sidebar, and — most
...
/** Build sidebar | (agent / shell) | editor — the sidebar plus a center pane
 *  with a nested pane and a document pane, matching the real dock shape. */
...
describe('toggleMaximizedGroup (double-click focus mode)', () => {
  it('maximizes the focused pane, hiding other panes and the sidebar', async () => {
    ...
    expect(isGroupVisible(dv, 'sidebar')).toBe(false)
    expect(isGroupVisible(dv, 'editor')).toBe(false)
    expect(isGroupVisible(dv, 'shell')).toBe(false)
    expect(isGroupVisible(dv, 'agent')).toBe(true)
  })

  it('restores every pane and the sidebar on the second toggle', async () => {
    const dv = await setupDock()
    ...
    expect(isGroupVisible(dv, 'sidebar')).toBe(true)
    expect(isGroupVisible(dv, 'editor')).toBe(true)
  ...
  it('never remounts panels across a maximize/restore round-trip', async () => {
    const dv = await setupDock()
    expect(mounts).toMatchObject({ agent: 1, sidebar: 1, editor: 1, shell: 1 })
